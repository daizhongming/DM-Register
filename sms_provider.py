...
NEXSMS_BASE_URL = "https://sms.nextactionplus.com/api/v1"
NEXSMS_DEFAULT_SERVICE = "openai"
NEXSMS_DEFAULT_COUNTRY = "US"

# SmsBower/sms-activate numeric country IDs used by the WebUI -> ISO2 codes used by NexSMS.
# Keep this table conservative; unsupported IDs fail clearly instead of sending a bad country.
NEXSMS_COUNTRY_MAP: dict[str, str] = {
    "0": "RU",
    "1": "UA",
    "2": "KZ",
    "3": "CN",
...
NEXSMS_SERVICE_MAP: dict[str, str] = {
    "dr": "openai",
    "openai": "openai",
    "chatgpt": "openai",
}
...
class NexSmsProvider(BaseSmsProvider):
    """NexSMS (sms.nextactionplus.com) provider."""

    BASE_URL = NEXSMS_BASE_URL
    ...
    def __init__(
        ...
        api_key: str,
        *,
        default_service: str = NEXSMS_DEFAULT_SERVICE,
        default_country: str = NEXSMS_DEFAULT_COUNTRY,
        pricing_option: int = 0,
        proxy: Optional[str] = None,
    ):
        self.api_key = str(api_key or "").strip()
        self.default_service = self._normalize_service(default_service)
        self.default_country = str(default_country or NEXSMS_DEFAULT_COUNTRY).strip() or NEXSMS_DEFAULT_COUNTRY
        self.pricing_option = max(0, _safe_int(pricing_option, 0))
        self._proxy = (proxy or "").strip() or None
        self._proxies = {"http": self._proxy, "https": self._proxy} if self._proxy else None
        self._resend_callback: Optional[Callable[[], None]] = None
        self.last_code_result: Optional[dict] = None
    ...
    def _normalize_service(service: str) -> str:
        ...
        return NEXSMS_SERVICE_MAP.get(value, value or NEXSMS_DEFAULT_SERVICE)

    @staticmethod
    def _normalize_country(country: str) -> str:
        value = str(country or "").strip()
        if not value:
            return NEXSMS_DEFAULT_COUNTRY
        upper = value.upper()
        if len(upper) == 2 and upper.isalpha():
            return upper
        mapped = NEXSMS_COUNTRY_MAP.get(value)
        if mapped:
            return mapped
        raise ValueError(f"NexSMS country id is not mapped to ISO2: {country}")
    ...
    @staticmethod
    def _extract_order(data) -> dict:
        if not isinstance(data, dict):
            raise RuntimeError("NexSMS response is not a JSON object")
        direct = data.get("order")
        ...
        if isinstance(nested, dict):
            if isinstance(nested.get("order"), dict):
                return nested["order"]
            orders = nested.get("orders")
            if isinstance(orders, list) and orders and isinstance(orders[0], dict):
                return orders[0]
            if nested.get("id") or nested.get("order_id") or "last_code" in nested:
                return nested
        orders = data.get("orders")
        if isinstance(orders, list) and orders and isinstance(orders[0], dict):
            return orders[0]
        if data.get("id") or data.get("order_id") or "last_code" in data:
            return data
        raise RuntimeError(f"NexSMS response has no order object: {str(data)[:300]}")

    def _request(self, method: str, path: str, *, timeout: int = 30, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise RuntimeError(f"NexSMS {method} {path} failed: {resp.status_code} {body}") from exc
        return resp

    def get_balance(self) -> float:
        data = self._request("GET", "/profile", timeout=10).json()
        if isinstance(data, dict) and isinstance(data.get("profile"), dict):
            data = data["profile"]
        if isinstance(data, dict) and "balance_cents" in data:
            return float(data["balance_cents"]) / 100
        if isinstance(data, dict) and "balance" in data:
            return float(data["balance"])
        raise RuntimeError(f"NexSMS /profile response has no balance: {str(data)[:300]}")

    def get_number(self, *, service: str, country: str = "",
                    country_candidates: Optional[list[str]] = None) -> SmsActivation:
        service_code = self._normalize_service(service or self.default_service)
        candidates = country_candidates or [country or self.default_country]
        failures: list[str] = []
        last_exc: Optional[Exception] = None

        for raw_country in candidates:
            try:
                iso_country = self._normalize_country(raw_country)
                payload = {
                ...
                resp = self._request("POST", "/orders", json=payload, timeout=15)
                order = self._extract_order(resp.json())
                activation_id = str(
                    order.get("id") or order.get("order_id") or order.get("activation_id") or ""
                ).strip()
                ...
                if not activation_id or not phone.strip("+"):
                    failures.append(f"{raw_country}: missing id/phone")
                    continue
                activation = SmsActivation(
                    activation_id=activation_id,
                    phone_number=phone,
                    country=iso_country,
                    metadata={"raw_country": raw_country, "pricing_option": self.pricing_option},
                )
                self.current_activation = activation
                logger.info(
                    "NexSMS rented number: service=%s country=%s phone=%s order_id=%s",
                    service_code, iso_country, phone, activation_id,
                )
                return activation
            except Exception as exc:
                failures.append(f"{raw_country}: {str(exc)[:160]}")
                last_exc = exc

        detail = " | ".join(failures) if failures else "unknown"
        raise RuntimeError(f"NexSMS failed to rent number: {detail}") from last_exc

    def get_status(self, activation_id: str) -> dict:
        data = self._request("GET", f"/orders/{activation_id}", timeout=10).json()
        order = self._extract_order(data)
        code = order.get("last_code") or order.get("code") or order.get("sms_code")
        candidate = _make_sms_candidate(activation_id, "nexsms.last_code", code)
        if candidate:
            return candidate
        status = str(order.get("status") or "").strip().lower()
        if status in {"cancel", "canceled", "cancelled", "expired", "refunded"}:
            return {"status": "cancel", "raw": status}
        return {"status": "wait_code", "raw": status}

    def request_resend_sms(self, activation_id: str) -> bool:
        try:
            self._request("POST", f"/orders/{activation_id}/resend", timeout=10)
            return True
        except Exception as exc:
            logger.debug("NexSMS resend failed: %s", exc)
            return False
    ...
    def wait_for_code(self, activation_id: str, *, timeout: int = 80, poll: int = 3,
        ...
        while time.time() < deadline:
            try:
                ...
                logger.debug("NexSMS status failed: %s", exc)

            elapsed = time.time() - start
            expected_resend_count = min(openai_resend_max, int(elapsed // openai_resend_interval))
            if expected_resend_count > openai_resend_count:
                if self._resend_callback:
                    try:
                        self._resend_callback()
                    except Exception as exc:
                        logger.warning("OpenAI resend callback failed: %s", exc)
                openai_resend_count = expected_resend_count
                self.request_resend_sms(activation_id)
                last_provider_resend = time.time()
            elif time.time() - last_provider_resend >= openai_resend_interval:
                self.request_resend_sms(activation_id)
                last_provider_resend = time.time()
    ...
    def cancel(self, activation_id: str) -> bool:
        try:
            self._request("POST", f"/orders/{activation_id}/cancel", timeout=10)
            return True
        except Exception as exc:
            logger.debug("NexSMS cancel failed: %s", exc)
            return False
...
def create_sms_provider(provider_key: str, config: dict) -> BaseSmsProvider:
    ...
    if pk in ("nexsms", "nex_sms"):
        return NexSmsProvider(api_key=api_key,
                              default_service=service or NEXSMS_DEFAULT_SERVICE,
                              default_country=country or NEXSMS_DEFAULT_COUNTRY,
                              pricing_option=_safe_int(config.get("sms_pricing_option"), 0),
                              proxy=proxy)
