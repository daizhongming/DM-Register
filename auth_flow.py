...
class AuthFlow:
    ...
    def _follow_authorize_for_callback(
        self,
        start_url: str,
        redirect_uri: str,
        trace_prefix: str,
        ...
        selected_target_workspace = False
        for i in range(12):
            ...
            if resp.status_code == 200:
                ...
                if is_workspace_like:
                    ...
                    if target_ws and not selected_target_workspace:
                        selected_target_workspace = True
                        next_url = self._workspace_select(target_ws)
                        if next_url:
    ...
    def oauth_codex_rt_exchange(
        ...
        try:
            ...
            callback_url, final_url = self._follow_authorize_for_callback(
                auth_url, redirect_uri, "codex_authorize", target_workspace_id=target_workspace_id
            )
    ...
    def _rotate_impersonate_session(self) -> bool:
        ...
        try:
            ...
            logger.debug("failed to carry cookies after impersonate rotate", exc_info=True)
        return True
    ...
    def signup(self, email: str, sentinel_token: str) -> bool:
        ...
        try:
            response_type = str(data.get("type") or "").strip().lower() if isinstance(data, dict) else ""
            if response_type in ("passwordless_signup", "passwordless_login"):
                self._is_existing_account = True
                self._existing_email_verification_mode = response_type
                self._existing_page_type = "email_otp_verification"
                logger.info("检测到 passwordless 已有账号，切换到 OTP 登录流程")
                return False
            ...
            if page_type == "email_otp_verification":
                ...
                self._existing_page_type = page_type
                if self._existing_email_verification_mode.lower() in ("passwordless_signup", "passwordless_login"):
                    logger.info("检测到 passwordless 已有账号，切换到 OTP 登录流程")
    ...
    def run_register(self, mail_provider: MailProvider) -> AuthResult:
        ...
        self.result.created_new = bool(is_new)
        self.result.login_mode = not is_new

        is_outlook_pool = (not is_new and hasattr(mail_provider, "_outlook_creds")
                           and bool(mail_provider._outlook_creds))
        if is_outlook_pool:
            logger.info(f"[outlook] '已有账号' 分支 → 走 OTP login 拿凭证 ({email})")
