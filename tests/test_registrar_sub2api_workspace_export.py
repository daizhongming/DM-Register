...
def test_exports_each_joined_workspace_once():
    ...
    try:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            ...
            assert db.get_k12_usable_workspace_ids_by_mail_source()["outlook"] == [WS1, WS2]
    finally:
...
def test_failed_workspace_export_is_not_recorded():
    old_db_path = db.DB_PATH
    old_get_export_internal_config = db.get_export_internal_config
    old_export_to_sub2api = exporter.export_to_sub2api
    try:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            db.DB_PATH = Path(tmp) / "webui.db"
            db.init_db()
            db.get_export_internal_config = lambda: {"sub2api": {"enabled": True, "sub2api_group_ids": "2"}}
            exporter.export_to_sub2api = lambda *args, **kwargs: {"ok": False, "error": "boom"}

            registrar._try_export_joined_workspaces_to_sub2api(
                "run-id",
                {"email": "test@example.com", "access_token": _jwt(WS1)},
                {
                    "joined_workspace_ids": [WS1],
                    "workspace_accounts_checked": True,
                    "workspace_accounts": {WS1: {"id": WS1}},
                },
                mail_source="paymesh_card",
            )

            assert db.get_k12_usable_workspace_ids_by_mail_source()["paymesh_card"] == []
    finally:
        db.DB_PATH = old_db_path
        db.get_export_internal_config = old_get_export_internal_config
        exporter.export_to_sub2api = old_export_to_sub2api
...
if __name__ == "__main__":
    ...
    test_failed_workspace_export_is_not_recorded()
