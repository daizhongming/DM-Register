...
def validate_k12_workspace_token(cred: dict, workspace_id: str) -> tuple[bool, str]:
    """Validate that the bearer token itself is scoped to the target K12 workspace."""
    ...
    account_id = str(auth.get("chatgpt_account_id") or auth.get("account_id") or "").strip().lower()
    plan_type = str(auth.get("chatgpt_plan_type") or auth.get("plan_type") or "").strip().lower()
    if plan_type != "k12":
        return False, f"access_token plan_type={plan_type or 'missing'}"
    if account_id != workspace_id:
        return False, f"access_token account_id={account_id or 'missing'}"
    return True, ""
...
def _split_group_refs(raw: Any) -> list[str]:
...
def _parse_group_ids(raw: Any, fallback: list[int] | None = None) -> list[int]:
    ...
    out: list[int] = []
    invalid: list[str] = []
    for text in _split_group_refs(raw):
        try:
            out.append(int(text))
        except ValueError:
            invalid.append(text)
    if invalid:
        raise ValueError(f"SUB2API 分组 IDs 必须是数字，当前无效: {', '.join(invalid)}")
...
def _parse_optional_positive_int(raw: Any, field_name: str) -> Optional[int]:
    ...
    try:
        ...
        raise ValueError(f"{field_name} 必须是正整数，当前无效: {text}")
    if value <= 0:
        raise ValueError(f"{field_name} 必须是正整数，当前无效: {text}")
    return value


def _extract_sub2api_list(data: Any) -> list:
    items = data
    if isinstance(data, dict):
        for key in ("data", "items", "rows"):
            ...
            if isinstance(value, dict):
                for nested_key in ("data", "items", "rows"):
...
def _resolve_sub2api_group_ids(
    cfg: dict,
    cffi,
    api_url: str,
    api_key: str,
    ...
    raw = cfg.get("sub2api_group_ids")
    try:
        return _parse_group_ids(raw)
    except ValueError:
        refs = _split_group_refs(raw)
        if not refs:
            return list(DEFAULT_SUB2API_GROUP_IDS)

    resp = cffi.get(
        f"{api_url}/api/v1/admin/groups/all",
        headers={
    ...
    if resp.status_code not in (200, 201):
        raise ValueError(f"SUB2API group names need /api/v1/admin/groups/all, got HTTP {resp.status_code}")

    groups = _extract_sub2api_list(resp.json())
    if not isinstance(groups, list):
        raise ValueError("SUB2API groups response is not a list")

    by_name: dict[str, int] = {}
    for group in groups:
        if not isinstance(group, dict):
            continue
        raw_id = group.get("id", group.get("ID", group.get("group_id")))
        try:
            group_id = int(raw_id)
        except (TypeError, ValueError):
            continue
        name = str(group.get("name") or group.get("Name") or group.get("title") or "").strip()
        if name:
            by_name[name.lower()] = group_id
    ...
    for ref in refs:
        try:
            ...
            pass
        group_id = by_name.get(ref.lower())
        if group_id is None:
            missing.append(ref)
        else:
            out.append(group_id)
    if missing:
        raise ValueError(f"SUB2API groups not found: {', '.join(missing)}")
    log(f"[SUB2API] resolved group refs {refs} -> {out}", "info")
    return out or list(DEFAULT_SUB2API_GROUP_IDS)


def _resolve_sub2api_proxy_id(
    raw: Any,
    cffi,
    api_url: str,
    api_key: str,
    ...
    if not text:
        return None
    try:
        return _parse_optional_positive_int(text, "SUB2API proxy")
    except ValueError:
        pass

    resp = cffi.get(
        f"{api_url}/api/v1/admin/proxies/all",
        headers={
            "Accept": "application/json, text/plain, */*",
            "Referer": f"{api_url}/admin/proxies",
            "x-api-key": api_key,
        },
        proxies=None,
        verify=False,
    ...
    if resp.status_code not in (200, 201):
        raise ValueError(f"SUB2API proxy name needs /api/v1/admin/proxies/all, got HTTP {resp.status_code}")

    matches: list[int] = []
    wanted = text.lower()
    for proxy in _extract_sub2api_list(resp.json()):
        if not isinstance(proxy, dict):
            continue
        name = str(proxy.get("name") or proxy.get("Name") or "").strip()
        if name.lower() != wanted:
            continue
        try:
            matches.append(int(proxy.get("id", proxy.get("ID"))))
        except (TypeError, ValueError):
            pass

    if not matches:
        raise ValueError(f"SUB2API proxy not found: {text}")
    if len(matches) > 1:
        raise ValueError(f"SUB2API proxy name is duplicated: {text}")
    log(f"[SUB2API] resolved proxy {text!r} -> {matches[0]}", "info")
    return matches[0]
...
def build_sub2api_payload(cred: dict, group_ids: list[int]) -> dict:
    ...
    # organization_id 优先用调用方传入的 workspace org；没有再从 token 抽。
    id_auth = _get_auth(_decode_jwt_payload(id_token))
    organization_id = str(cred.get("organization_id") or "").strip()
    ...
    chatgpt_user_id = str(cred.get("chatgpt_user_id") or access_auth.get("chatgpt_user_id") or "").strip()

    # 从 workspace_account 中提取 plan_type 和 workspace_id（如果有）
    workspace_account = cred.get("workspace_account")
    plan_type = str(cred.get("plan_type") or "").strip()
    workspace_id_value = str(cred.get("workspace_id") or "").strip()

    if workspace_account and isinstance(workspace_account, dict):
        plan_type = plan_type or str(workspace_account.get("planType") or "").strip()
        workspace_id_value = workspace_id_value or str(workspace_account.get("id") or "").strip()
    plan_type = plan_type or str(access_auth.get("chatgpt_plan_type") or "").strip()

    account_name = str(cred.get("sub2api_name") or email).strip() or email
    ...
    if workspace_id_value:
        credentials["workspace_id"] = workspace_id_value
...
def _parse_sub2api_create_response(resp) -> tuple[bool, str, str]:
    """返回 (ok, account_id, error_message)。SUB2API 成功也可能包在 {code,data} 里。"""
    body_preview = ""
    try:
        body_preview = (resp.text or "")[:400]
    except Exception:
    ...
    if resp.status_code in (200, 201):
        if isinstance(data, dict) and "code" in data:
            ...
            if not code_ok:
                msg = str(data.get("message") or data.get("msg") or data.get("error") or "").strip()
                return False, "", msg or f"SUB2API code={data.get('code')}: {body_preview}"
            account = data.get("data")
        else:
            account = data

        if isinstance(account, dict):
            new_id = str(account.get("id") or account.get("ID") or "").strip()
        else:
            new_id = ""
        return True, new_id, ""
    ...
    msg = f"HTTP {resp.status_code}"
    if isinstance(data, dict):
        msg = str(data.get("message") or data.get("msg") or data.get("error") or msg)
    elif body_preview:
        msg = f"{msg}: {body_preview}"
    return False, "", msg
...
def export_to_sub2api(cred: dict, cfg: dict, *,
    ...
    cffi = _import_cffi()
    group_ids = _resolve_sub2api_group_ids(cfg, cffi, api_url, api_key, timeout, log)

    payload = build_sub2api_payload(cred, group_ids)
    payload_creds = payload.get("credentials") or {}
    plan_type = str(payload_creds.get("plan_type") or "").strip().lower()
    workspace_id = str(payload_creds.get("workspace_id") or "").strip()
    if plan_type == "k12":
        target_account_id = str(payload_creds.get("chatgpt_account_id") or workspace_id).strip()
        valid, reason = validate_k12_workspace_token(payload_creds, target_account_id)
        if not valid:
            email = payload.get("name") or str(cred.get("email") or "unknown")
            msg = f"{email} is not a workspace-scoped k12 token: {reason}"
            log(f"[SUB2API] {msg}", "error")
            return {"ok": False, "error": msg, "email": email}
    if not workspace_id and plan_type in ("", "free", "personal"):
        email = payload.get("name") or str(cred.get("email") or "unknown")
        msg = f"{email} is free tier, skipping export"
        log(f"[SUB2API] {msg}", "error")
        return {"ok": False, "error": msg, "email": email}

    proxy_ref = cred.get("sub2api_proxy_id") or cfg.get("sub2api_proxy_id")
    proxy_id = _resolve_sub2api_proxy_id(proxy_ref, cffi, api_url, api_key, timeout, log)
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            log(
                ...
                f"(group_ids={group_ids}, proxy_id={proxy_id or 'none'}, "
                f"plan_type={plan_type or 'unknown'}, has_workspace_id={bool(workspace_id)})...",
                "info",
            )
            resp = cffi.post(
                url,
            ...
            ok, new_id, msg = _parse_sub2api_create_response(resp)
            if ok:
...
def run_exports(cred: dict, *,
    ...
    # 如果没有 refresh_token，跳过刷新，直接用现有 access_token 导出
    ...
    if refresh_token:
        try:
            ...
            log(f"[exporter] ⚠️ Codex token 刷新失败，将使用原 access_token 导出: {e}", "warn")
            # 不返回，继续用原 access_token 尝试导出
    else:
        log("[exporter] 未提供 refresh_token，将直接使用 access_token 导出", "info")
