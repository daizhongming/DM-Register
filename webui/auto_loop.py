...
class AutoLoopController:
    ...
    def _worker_loop(self, worker_id: int):
        ...
        while True:
            ...
            # claim 下一个号（CF/PayMesh 模式用虚拟占位，无需 outlook 号池）
            mail_source = db.get_setting("mail_source", "outlook")
            if mail_source in ("cf_temp", "paymesh_card"):
                if mail_source == "paymesh_card" and db.paymesh_card_stats().get("available", 0) <= 0:
                    account = None
                else:
                    prefix = "paymesh_placeholder" if mail_source == "paymesh_card" else "cf_placeholder"
                    account = {
                        "email": f"{prefix}_{int(time.time())}_{worker_id}@local",
            ...
            try:
                ...
                if mail_source not in ("cf_temp", "paymesh_card"):
